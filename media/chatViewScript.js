...
(function () {
    if (typeof acquireVsCodeApi === 'undefined') {
        console.error("acquireVsCodeApi is not available. Make sure this script is run within a VS Code webview context.");

        document.body.innerHTML = '<p style="padding: 1em; color: var(--vscode-errorForeground);">Error: Chat UI could not initialize VS Code API.</p>';
        return;
    }
    ...
    if (!responseDiv || !promptInput || !modelNameSpan || !sendButton) {
        console.error("ChatView Error: One or more essential UI elements not found in the HTML (check IDs: response, prompt, modelName, send-button).");

        document.body.innerHTML = '<p style="padding: 1em; color: var(--vscode-errorForeground);">Error: Chat UI elements are missing.</p>';
        return;
    }
    ...
    function appendToResponse(text, isPrompt = false) {
        ...
        const formattedText = (responseDiv.innerHTML.trim() ? '\n' : '') + prefix + text + (isPrompt ? '\n' : '');
    ...
    function handleUpdateModel(message) {
        ...
        console.log(`[ChatView] handleUpdateModel: Received model='${currentModel}', hasValidModel=${hasValidModel}`);

        if (!modelNameSpan || !promptInput || !sendButton) {
            console.error("[ChatView] handleUpdateModel: ERROR - UI elements not found!");
            return;
        }
        ...
        console.log(`[ChatView] handleUpdateModel: UI updated. Input disabled=${promptInput.disabled}, Button disabled=${sendButton.disabled}`);
    ...
    function handleSetThinking(message) {
        const thinking = message.thinking;
         ...
         if (!promptInput || !sendButton || !modelNameSpan) {
            console.error("[ChatView] handleSetThinking: ERROR - UI elements not found!");
            return;
        }
        ...
        if (thinkingIndicator) {
         ...
         console.log(`[ChatView] handleSetThinking: UI updated. Input disabled=${promptInput.disabled}, Button disabled=${sendButton.disabled}`);
    }
    ...
    function handleLoadChat(message) {
        ...
        if (modelNameSpan) {modelNameSpan.textContent = `(${displayModelName})`;}
        if (promptInput) {
             promptInput.disabled = !hasValidModel;
             promptInput.placeholder = hasValidModel ? 'Enter your prompt...' : 'Select a model...';
        }
        if (sendButton) {sendButton.disabled = !hasValidModel;}
    ...
    window.addEventListener('message', event => {
        ...
        switch (message.command) {
            ...
            case 'clearDisplay': handleClearDisplay(); break;
