...
export class ModelManagerViewProvider implements vscode.WebviewViewProvider {
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(message => {
            ...
            switch (message.command) {
                 case 'load': vscode.commands.executeCommand('modelManager.load'); return;
                 case 'add': vscode.commands.executeCommand('modelManager.add'); return;
                 case 'import': vscode.commands.executeCommand('modelManager.import'); return;
                 case 'delete': vscode.commands.executeCommand('modelManager.delete'); return;
                 case 'getModel': this.updateView(); return;
                 case 'log': console.log("Manager Webview Log:", message.data); return;
                 case 'openChat':
                    vscode.commands.executeCommand('ollamate-ext.start');
                    return;
                 default: console.warn("Received unknown command from manager webview:", message.command);
            }
        });
    ...
    public updateView() {
        if (this._view) {
            ...
            console.log(`Updating manager view. Selected: ${selectedModel}, Available: ${availableModels.length}`);
